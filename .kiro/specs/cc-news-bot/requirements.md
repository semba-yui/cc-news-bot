...
- 特殊な用語などあればピックアップして解説（なければこのセクションごと省略）
```
...
3. The Changelog Notifier shall 以下のフォーマットで要約を生成する:
   ...
   - 「変更内容」セクション: 「破壊的変更」「セキュリティ」「新規追加」「修正」「改善」「パフォーマンス」「削除」「非推奨」「その他」のサブセクションに分類し、該当するカテゴリのみ出力する
   - 「用語解説」セクション: 特殊な用語があればピックアップして解説（なければ省略）
