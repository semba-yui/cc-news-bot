...
export async function fetchHtmlOpenAINews(
  ...
  let listHtml: string;
  try {
    listHtml = await deps.fetchHeadlessHtml(LIST_URL, { timeoutMs: 60_000 });
  } catch (err) {
  ...
  for (const article of newArticles) {
    ...
    try {
      articleHtml = await deps.fetchHeadlessHtml(`${ARTICLE_BASE_URL}${article.slug}/`, { timeoutMs: 60_000 });
    } catch (err) {
      const message = err instanceof Error ? err.message : String(err);
