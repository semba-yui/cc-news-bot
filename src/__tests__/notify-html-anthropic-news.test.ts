...
import {
  notifyHtmlAnthropicNews,
  type NotifyHtmlAnthropicNewsDeps,
} from "../scripts/notify-html-anthropic-news.js";

// What: Anthropic News の Slack 通知スクリプトのテスト
// Why: 翻訳・要約済み JSON を読み込み、記事ごとに親メッセージ（要約）+
//      スレッド返信（全文）を Slack に投稿するフローが正しく動作することを保証する
//      OpenAI News と同じ要約 + スレッド全文パターン

const TEST_ROOT = resolve(import.meta.dirname, "../../data-test-notify-anthropic-news");

const MOCK_BLOCKS: SlackBlock[] = [
  {
    type: "header",
    text: { type: "plain_text", text: "Anthropic News: テスト記事", emoji: true },
  },
...
function makeDeps(
  overrides: Partial<NotifyHtmlAnthropicNewsDeps> = {},
): NotifyHtmlAnthropicNewsDeps {
  return {
...
function writeSummaryFile(data: SummaryEntry[]): void {
  writeFileSync(
    resolve(TEST_ROOT, "html-summaries", "anthropic-news.json"),
    JSON.stringify(data, null, 2),
  );
}

describe("notifyHtmlAnthropicNews", () => {
  beforeEach(() => {
  ...
  describe("単一記事の投稿", () => {
    ...
    it("親メッセージに要約を Block Kit で投稿し、スレッド返信に全文を投稿する", async () => {
      ...
      // When: 通知スクリプトを実行する
      await notifyHtmlAnthropicNews(deps);
